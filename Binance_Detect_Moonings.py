...
def pause_bot():
    ...
    while os.path.isfile("signals/paused.exc"):
        ...
        if bot_paused:
            print(f'{txcolors.WARNING}Resuming buying due to change in market conditions,'
                  f' total sleep time: {time_elapsed}{txcolors.DEFAULT}')
            bot_paused = False
...
def sell_coins():
    ...
    last_price = get_price(add_to_historical=False)  # don't populate rolling window
    ...
    for coin in list(coins_bought):
        ...
        if LastPrice > TP and USE_TRAILING_STOP_LOSS:
            ...
            if DEBUG:
                print(f"{coin} TP reached, adjusting TP {coins_bought[coin]['take_profit']:.2f}  and"
                      f" SL {coins_bought[coin]['stop_loss']:.2f} accordingly to lock-in profit")
            continue
        ...
        if LastPrice < SL or LastPrice > TP and not USE_TRAILING_STOP_LOSS:
            ...
            # run the else block if coin has been sold and create a dict for each coin sold
                ...
                if LOG_TRADES:
                    ...
                    write_log(f"Sell: {coins_sold[coin]['volume']} {coin} - {BuyPrice} - {LastPrice}"
        ...
        if hsp_head == 1:
            if len(coins_bought) > 0:
                print(f'TP or SL not yet reached, not selling {coin} for now {BuyPrice} - {LastPrice} :'
                      f' {txcolors.SELL_PROFIT if PriceChange >= 0. else txcolors.SELL_LOSS}'
                      f'{PriceChange - (TRADING_FEE * 2):.2f}% Est:$'
                      f'{(QUANTITY * (PriceChange - (TRADING_FEE * 2))) / 100:.2f}{txcolors.DEFAULT}')
...
if __name__ == '__main__':
    ...
    try:
        if len(SIGNALLING_MODULES) > 0:
            for module in SIGNALLING_MODULES:
                ...
                t = threading.Thread(target=mymodule[module].do_work, args=([client]))
                t.daemon = True
                t.start()
                time.sleep(2)
        else:
    ...
    while True:
        try:
            ...
            print(f'{txcolors.WARNING}We got a timeout error from from binance.'
                  f' Going to re-loop. Current Count: {READ_TIMEOUT_COUNT}\n{rt}{txcolors.DEFAULT}')
        except ConnectionError as ce:
            CONNECTION_ERROR_COUNT += 1
            print(f'{txcolors.WARNING}We got a timeout error from from binance.'
                  f' Going to re-loop. Current Count: {CONNECTION_ERROR_COUNT}\n{ce}{txcolors.DEFAULT}')
