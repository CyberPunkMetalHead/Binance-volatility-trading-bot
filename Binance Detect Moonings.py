    ...
    while initial_price['BNB' + PAIR_WITH]['time'] > datetime.now() - timedelta(seconds=TIME_DIFFERENCE):
        ...
        while i < RECHECK_INTERVAL:
            if len(coins_bought) > 0:
                print(f'checking TP/SL...Number of coins: {len(coins_bought)}')
        ...
        for coin in initial_price:
            if float(last_price[coin]['price']) > float(initial_price[coin]['price']):
                coinsUp +=1
            elif float(last_price[coin]['price']) < float(initial_price[coin]['price']):
                coinsDown +=1
            else:
                coinsUnchanged +=1
            ...
            if threshold_check > CHANGE_IN_PRICE:
                if len(coins_bought) < MAX_COINS or MAX_COINS == 0:
                    ...
                    print(f'{txcolors.WARNING}{coin} has gained {threshold_check:.3f}% in the last {TIME_DIFFERENCE} seconds, but you are holding max number of coins{txcolors.DEFAULT}')
        ...
        print(f'Up: {coinsUp} Down: {coinsDown} Unchanged: {coinsUnchanged}')
...
def buy():
    ...
    for coin in volume:
        ...
        # only buy if the there are no active trades on the coin and coins bought are less than MAX_COINS
        if coin not in coins_bought and (len(coins_bought)+num_new_coins < MAX_COINS):
            ...
            print(f'{txcolors.WARNING}Signal detected, but you already hold {coin} or cant hold more coins{txcolors.DEFAULT}')
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if float(last_price[coin]['price']) > TP and USE_TRAILING_STOP_LOSS:
            ...
            print(f"{txcolors.WARNING}{coin} TP reached, adjusting TP and SL accordingly TP:{coins_bought[coin]['take_profit']:.2f}% SL:{coins_bought[coin]['stop_loss']:.2f}%{txcolors.DEFAULT}")
