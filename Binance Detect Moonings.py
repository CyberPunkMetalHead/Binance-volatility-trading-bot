...
def get_price():
    ...
    for coin in prices:
        ...
        if CUSTOM_LIST:
            if any(
                item + PAIR_WITH == coin["symbol"] for item in tickers
            ) and all(item not in coin["symbol"] for item in FIATS):
...
def wait_for_price():
    ...
    while initial_price["BNB" + PAIR_WITH][
        "time"
    ] > datetime.now() - timedelta(seconds=TIME_DIFFERENCE):
        i = 0
        ...
        for coin in initial_price:
            threshold_check = (
                (
                    ...
                    - float(initial_price[coin]["price"])
                )
                / float(initial_price[coin]["price"])
                * 100
            )
        ...
        if infoChange < CHANGE_IN_PRICE:
            print(
                f"No coins moved more than {CHANGE_IN_PRICE}% in the last {TIME_DIFFERENCE} second(s)"
            )
    ...
    for coin in volatile_coins:
        ...
        if coin not in lot_size:
            ...
            if lot_size[coin] == 0:
                ...
                volume[coin] = float(
                    "{:.{}f}".format(volume[coin], lot_size[coin])
                )
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if float(last_price[coin]["price"]) > TP and USE_TRAILING_STOP_LOSS:
            print(
                "TP reached, adjusting TP and SL accordingly to lock-in profit"
            )
        ...
        if float(last_price[coin]["price"]) < SL or (
            float(last_price[coin]["price"]) > TP
            and not USE_TRAILING_STOP_LOSS
        ):
            print(
                f"{txcolors.SELL}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {BuyPrice} - {LastPrice} : {PriceChange:.2f}%{txcolors.DEFAULT}"
            )
            ...
            # run the else block if coin has been sold and create a dict for each coin sold
                ...
                if LOG_TRADES:
                    profit = (LastPrice - BuyPrice) * coins_sold[coin][
                        "volume"
                    ]
                    write_log(
...
if __name__ == "__main__":
    ...
    USE_TRAILING_STOP_LOSS = parsed_config["trading_options"][
